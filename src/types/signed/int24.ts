...
class Int24 extends DataType {
  /**
   * The size of the Int24 data type in bytes.
  */
  public static readonly SIZE = 3;

  /**
   * Read an 24-bit signed integer (i24) from the stream.
  ...
  public static read(stream: BinaryStream, options?: DataTypeOptions): number {
    ...
    const byte1 = stream.buffer.readInt8(stream.offset++);
    const byte2 = stream.buffer.readInt8(stream.offset++);
    const byte3 = stream.buffer.readInt8(stream.offset++);

    // Combine the bytes into a 24-bit signed integer
  ...
  /**
   * Write an 24-bit signed integer (i24) to the stream.
  ...
  public static write(stream: BinaryStream, value: number, options?: DataTypeOptions): void {
    ...
    // Write the signed integer value as a 3-byte integer based on endianness
    ...
    if (options?.endian === undefined || options.endian === Endianness.Big) {
      stream.buffer.writeInt8(byte1, stream.offset++);
      stream.buffer.writeInt8(byte2, stream.offset++);
      stream.buffer.writeInt8(byte3, stream.offset++);
    } else {
      stream.buffer.writeInt8(byte3, stream.offset++);
      stream.buffer.writeInt8(byte2, stream.offset++);
      stream.buffer.writeInt8(byte1, stream.offset++);
