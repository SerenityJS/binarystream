...
class Int16 extends DataType {
  /**
   * The size of the Int16 data type in bytes.
  */
  public static readonly SIZE = 2;

  /**
   * Read an 16-bit signed integer (i16) from the stream.
  ...
  public static read(stream: BinaryStream, options?: DataTypeOptions): number {
    ...
    const value = (options?.endian === undefined || options.endian === Endianness.Big)
      ? stream.buffer.readInt16BE(stream.offset)
      : stream.buffer.readInt16LE(stream.offset);
  ...
  /**
   * Write an 16-bit signed integer (i16) to the stream.
  ...
  public static write(stream: BinaryStream, value: number, options?: DataTypeOptions): void {
    ...
    // Write the signed integer value as a 2-byte integer based on endianness
