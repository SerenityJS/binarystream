...
class Int32 extends DataType {
  /**
   * The size of the Int32 data type in bytes.
  */
  public static readonly SIZE = 4;

  /**
   * Read an 32-bit signed integer (i32) from the stream.
  ...
  public static read(stream: BinaryStream, options?: DataTypeOptions): number {
    ...
    const value = (options?.endian === undefined || options.endian === Endianness.Big)
      ? stream.buffer.readInt32BE(stream.offset)
      : stream.buffer.readInt32LE(stream.offset);
  ...
  /**
   * Write an 32-bit signed integer (i32) to the stream.
