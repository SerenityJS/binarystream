...
import { Uint32 } from "../unsigned";

class String32 extends DataType {
  /**
   * The size of the String32 data type in bytes.
  */
  ...
  public static read(stream: BinaryStream, options?: DataTypeOptions): string {
    ...
    const length = Uint32.read(stream, options);
  ...
  public static write(stream: BinaryStream, value: string, options?: DataTypeOptions): void {
    ...
    if (!stream.validateOffset(buffer.length + Uint32.SIZE)) {
      throw new Error('Write exceeds buffer length');
    }

    // Write the length of the string
    Uint32.write(stream, buffer.length, options);
