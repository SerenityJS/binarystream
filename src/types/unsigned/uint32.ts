...
class Uint32 extends DataType {
  /**
   * The size of the Uint32 data type in bytes.
  */
  public static readonly SIZE = 4;

  /**
   * Read an 32-bit unsigned integer (i32) from the stream.
  ...
  public static read(stream: BinaryStream, options?: DataTypeOptions): number {
    ...
    const value = (options?.endian === undefined || options.endian === Endianness.Big)
      ? stream.buffer.readUint32BE(stream.offset)
      : stream.buffer.readUint32LE(stream.offset);
  ...
  /**
   * Write an 32-bit unsigned integer (i32) to the stream.
  ...
  public static write(stream: BinaryStream, value: number, options?: DataTypeOptions): void {
    ...
    // Write the unsigned integer value as a 4-byte integer based on endianness
