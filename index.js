  ...
  case 'linux':
    switch (arch) {
      ...
      case 'arm':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'binarystream.linux-arm-musleabihf.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./binarystream.linux-arm-musleabihf.node')
            } else {
              nativeBinding = require('@serenityjs/binarystream-linux-arm-musleabihf')
            }
          } catch (e) {
            loadError = e
          }
